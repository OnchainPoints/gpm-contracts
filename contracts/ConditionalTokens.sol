...
contract ConditionalTokens is Ownable, ERC1155URIStorage{
    ...
    function redeemPositionsOnBehalf(
        IERC20 collateralToken, 
        bytes32 parentCollectionId, 
        bytes32 conditionId, 
        uint[] calldata indexSets,
        address redeemerAddress
    ) external onlyOracle returns (uint totalPayout) {
        uint den = payoutDenominator[conditionId];
        require(den > 0, "result for condition not received yet");
        uint outcomeSlotCount = payoutNumerators[conditionId].length;
        require(outcomeSlotCount > 0, "condition not prepared yet");
        ...
        for (uint i = 0; i < indexSets.length; i++) {
            ...
            uint payoutStake = balanceOf(redeemerAddress, positionId);
            if (payoutStake > 0) {
                totalPayout = totalPayout.add(payoutStake.mul(payoutNumerator).div(den));
                _burn(redeemerAddress, positionId, payoutStake);
            }
        }
        ...
        if (totalPayout > 0) {
            if (parentCollectionId == bytes32(0)) {
                require(collateralToken.transfer(oracleAddress, totalPayout), "could not transfer payout to redeemer address");
            } else {
                _mint(oracleAddress, CTHelpers.getPositionId(collateralToken, parentCollectionId), totalPayout, "");
            }
        }
        emit PayoutRedemption(redeemerAddress, collateralToken, parentCollectionId, conditionId, indexSets, totalPayout);
    ...
    // Update the URI for the ERC1155 token
